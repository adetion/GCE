...
"""事件总线 —— multiprocessing.Queue 封装 / Event bus — multiprocessing.Queue wrapper."""
...
from gce.environment.GCE_base import GCE_RawEvent


class GCE_EventBus:
    """包装 multiprocessing.Queue，用于发布和消费 RawEvent / Wraps a multiprocessing.Queue for publishing and consuming RawEvents."""

    def __init__(self, maxsize: int = 10000) -> None:
        self._queue: multiprocessing.Queue[GCE_RawEvent] = multiprocessing.Queue(maxsize=maxsize)

    @property
    def GCE_queue(self) -> multiprocessing.Queue:  # type: ignore[type-arg]
        return self._queue

    def GCE_publish(self, event: GCE_RawEvent, timeout: float = 1.0) -> bool:
        """向总线发布事件。队列满时返回 False / Publish an event to the bus. Returns False if queue is full."""
        try:
    ...
    def GCE_consume(self, timeout: float = 1.0) -> Optional[GCE_RawEvent]:
        """从总线消费事件。超时时返回 None / Consume an event from the bus. Returns None on timeout."""
        try:
            return self._queue.get(timeout=timeout)
        except Exception:
            return None

    def GCE_close(self) -> None:
