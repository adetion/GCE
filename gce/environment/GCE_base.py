...
"""环境层抽象基类和事件格式契约 / Abstract base class and event format contract for the environment layer."""
...
class GCE_EventType(str, Enum):
    """事件类型枚举：数值、文本或向量 / Event type enumeration: numeric, text, or vector."""
    NUMERIC = "numeric"
    TEXT = "text"
    VECTOR = "vector"
...
class GCE_RawEvent:
    """原始事件数据类，环境产生的不可变事件 / Raw event dataclass, an immutable event produced by the environment."""
    source_id: str
    event_type: GCE_EventType
...
class GCE_BaseEnvironment(ABC):
    """抽象环境，生成 GCE_RawEvent 序列 / Abstract environment that produces GCE_RawEvent sequences."""

    @abstractmethod
    def GCE_start(self, event_queue: multiprocessing.Queue) -> None:  # type: ignore[type-arg]
        """开始向队列中生产事件，由独立进程调用 / Start producing events into the queue. Called from a dedicated process."""

    @abstractmethod
    def GCE_stop(self) -> None:
        """优雅地停止环境 / Gracefully stop the environment."""
