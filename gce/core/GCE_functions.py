...
"""表达式树函数集 —— 仅包含通用数学运算符 / Expression tree function set -- generic math operators only."""
...
# 每个条目: (参数数量, 可调用对象) / Each entry: (arity, callable)
# 所有函数均包含溢出/除零保护 / All functions include overflow/divide-by-zero protection.
FUNCTIONS: Dict[str, Tuple[int, Callable[..., float]]] = {
    "add": (2, lambda a, b: a + b),
    "sub": (2, lambda a, b: a - b),
    "mul": (2, lambda a, b: a * b),
    "div": (2, lambda a, b: a / b if b != 0 else 0.0),
    "sin": (1, math.sin),
    "cos": (1, math.cos),
    "exp": (1, lambda x: math.exp(x) if x < 50 else math.exp(50)),
    "log": (1, lambda x: math.log(abs(x) + 1e-9)),
    "abs": (1, abs),
    "ifgt": (4, lambda a, b, c, d: c if a > b else d),
}


def GCE_get_arity(op_name: str) -> int:
    """返回函数的参数数量，未知则返回 0 / Return the arity of a function, or 0 if unknown."""
    entry = FUNCTIONS.get(op_name)
    return entry[0] if entry else 0


def GCE_call_function(op_name: str, args: list[float]) -> float:
    """按名称调用函数并传入参数 / Call a function by name with the given arguments."""
    fn = FUNCTIONS[op_name][1]
    return fn(*args)
