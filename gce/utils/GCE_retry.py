...
"""数据库操作重试装饰器 / Database operation retry decorator."""
...
def GCE_retry_on_lock(
    max_retries: int = 5,
    base_delay: float = 0.01,
) -> Callable[[F], F]:
    """装饰器：在sqlite3.OperationalError（锁定/繁忙）时以指数退避重试 / Decorator: retry on sqlite3.OperationalError (locked/busy) with exponential backoff."""

    def GCE_decorator(func: F) -> F:
        @functools.wraps(func)
        def GCE_wrapper(*args, **kwargs):  # type: ignore[no-untyped-def]
            last_exception = None
        ...
        return GCE_wrapper  # type: ignore[return-value]

    return GCE_decorator
