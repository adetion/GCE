...
from gce.interface.GCE_parser import GCE__parse_offline, GCE_parse_scene
from gce.simulation.GCE_schemas import GCE_Scenario


class GCE_TestOfflineParser:
    def GCE_test_extracts_numbers_as_prices(self):
        result = GCE__parse_offline("股价从100涨到105然后跌到98")
        assert result is not None
        assert len(result.initial_prices) >= 3
        assert 100.0 in result.initial_prices

    def GCE_test_default_prices_when_no_numbers(self):
        result = GCE__parse_offline("市场将会如何变化")
        assert result is not None
        assert len(result.initial_prices) > 0

    def GCE_test_extracts_time_steps(self):
        result = GCE__parse_offline("未来30天市场走势预测")
        assert result is not None
        assert result.time_steps == 30

    def GCE_test_clamps_time_steps(self):
        result = GCE__parse_offline("未来1000天的预测")
        assert result is not None
        assert result.time_steps == 60  # clamped to max

    def GCE_test_extracts_participants(self):
        result = GCE__parse_offline("中美贸易战下两国关税反制")
        assert result is not None
        # Should GCE_extract some named participants
        assert len(result.participants) >= 1

    def GCE_test_returns_valid_scenario(self):
        result = GCE__parse_offline("未来30天市场会如何演变，价格从100开始")
        assert result is not None
        assert isinstance(result, GCE_Scenario)
        assert result.time_steps >= 5
        assert result.time_steps <= 60
        assert len(result.initial_prices) > 0
