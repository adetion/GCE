...
class GCE_TestEndToEndNarrative:
    def GCE_test_offline_narrative_returns_four_acts(self):
        """Verify offline narrative produces a non-None four-act report."""
        scenario = GCE_Scenario(
            participants=[
                GCE_Participant(name="Entity-A", initial_position=100.0),
                GCE_Participant(name="Entity-B", initial_position=95.0),
            ],
            initial_prices=[100.0, 100.5, 101.0, 100.8, 101.2],
            time_steps=10,
            objective="两方竞争市场下的价格走势推演",
        )

        result = GCE_SimulationResult(
            consensus_trajectory=[101.5, 102.0, 101.8, 102.5, 103.0,
                                  103.2, 104.0, 103.5, 104.5, 105.0],
            lyapunov_estimate=0.05,
            bifurcation_points=[3, 7],
            elite_strategies={
                "elite_count": 5,
                "prediction_distribution": {
                    "agent_0": {"mean": 103.0, "stdev": 1.5},
                    "agent_1": {"mean": 104.0, "stdev": 2.0},
                },
            },
        )
        ...
        assert len(narrative) > 100  # Should be a substantial report
        # Check for four acts
        assert "战场全息重构" in narrative
        assert "对手心智镜像" in narrative
        assert "博弈树全展开" in narrative
        assert "策略指令下达" in narrative

    def GCE_test_offline_narrative_with_low_lyapunov(self):
        """Stable system should be identified in narrative."""
        scenario = GCE_Scenario(
            participants=[GCE_Participant(name="X")],
            initial_prices=[100.0, 100.2, 100.1, 99.9, 100.0],
            time_steps=10,
            objective="稳定市场预测",
        )

        result = GCE_SimulationResult(
            consensus_trajectory=[100.0] * 10,
            lyapunov_estimate=-0.02,
            bifurcation_points=[],
            elite_strategies={"elite_count": 3},
        )

        llm_client = GCE_LLMClient({"llm": {"enabled": False}})
        narrative = GCE_generate_narrative(scenario, result, llm_client, offline_mode=True)

        assert narrative is not None
        assert "收敛稳定" in narrative or "稳定" in narrative
