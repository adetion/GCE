...
from gce.utils.GCE_config import GCE_load_config, GCE_validate_config, GCE__replace_env_vars


class GCE_TestConfigLoading:
    def GCE_test_load_valid_config(self):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump({
                "evolution": {"population_initial": 100, "max_population": 2000, "initial_energy": 100.0,
                              "reproduce_threshold": 200.0, "reproduction_cost": 80.0,
                              "max_depth": 8, "min_nodes": 3, "max_nodes": 50,
                              "crossover_rate": 0.7, "mutation_rate_base": 0.2},
                "environment": {"simulator": {"tick_interval": 0.5, "num_market_agents": 10}},
                "database": {"path": "test.db"},
                "simulation": {"elite_count": 10, "future_steps": 30,
                               "perturbation_epsilon": 0.001, "divergence_threshold": 0.5},
                "logging": {"level": "INFO"},
            }, f)
            path = f.name

        try:
            config = GCE_load_config(path)
            assert config["evolution"]["population_initial"] == 100
            GCE_validate_config(config)  # Should not raise
        finally:
            os.unlink(path)

    def GCE_test_env_var_replacement(self):
        os.environ["TEST_VAR"] = "hello123"
        result = GCE__replace_env_vars("prefix_${TEST_VAR}_suffix")
        assert result == "prefix_hello123_suffix"
        del os.environ["TEST_VAR"]

    def GCE_test_env_var_missing(self):
        result = GCE__replace_env_vars("${NONEXISTENT_VAR_XYZ}")
        assert result == ""  # Replaced with empty string


class GCE_TestConfigValidation:
    def GCE_test_invalid_population(self):
        with pytest.raises(SystemExit):
            GCE_validate_config({
                "evolution": {"population_initial": 0, "max_population": 2000, "initial_energy": 100.0,
                              "reproduce_threshold": 200.0, "reproduction_cost": 80.0,
                              "max_depth": 8, "min_nodes": 3, "max_nodes": 50,
                              "crossover_rate": 0.7, "mutation_rate_base": 0.2},
                "environment": {"simulator": {"tick_interval": 0.5, "num_market_agents": 10}},
    ...
    def GCE_test_invalid_log_level(self):
        with pytest.raises(SystemExit):
            GCE_validate_config({
                "evolution": {"population_initial": 100, "max_population": 2000, "initial_energy": 100.0,
                              "reproduce_threshold": 200.0, "reproduction_cost": 80.0,
