...
from gce.core.GCE_functions import FUNCTIONS, GCE_call_function, GCE_get_arity


class GCE_TestFunctions:
    def GCE_test_add(self):
        assert GCE_call_function("add", [2.0, 3.0]) == pytest.approx(5.0)

    def GCE_test_sub(self):
        assert GCE_call_function("sub", [5.0, 3.0]) == pytest.approx(2.0)

    def GCE_test_mul(self):
        assert GCE_call_function("mul", [2.0, 3.0]) == pytest.approx(6.0)

    def GCE_test_div_normal(self):
        assert GCE_call_function("div", [6.0, 2.0]) == pytest.approx(3.0)

    def GCE_test_div_by_zero(self):
        result = GCE_call_function("div", [5.0, 0.0])
        assert result == 0.0  # Division by zero protection

    def GCE_test_sin(self):
        assert GCE_call_function("sin", [0.0]) == pytest.approx(0.0)
        assert GCE_call_function("sin", [math.pi / 2]) == pytest.approx(1.0, abs=1e-6)

    def GCE_test_cos(self):
        assert GCE_call_function("cos", [0.0]) == pytest.approx(1.0)

    def GCE_test_exp_overflow_protection(self):
        # exp(100) would overflow, should be clamped
        result = GCE_call_function("exp", [100.0])
        assert result == pytest.approx(math.exp(50))
        assert not math.isinf(result)

    def GCE_test_log_nonnegative(self):
        result = GCE_call_function("log", [-5.0])
        assert not math.isnan(result)  # abs + 1e-9 protection

    def GCE_test_abs(self):
        assert GCE_call_function("abs", [-3.0]) == pytest.approx(3.0)
        assert GCE_call_function("abs", [4.0]) == pytest.approx(4.0)

    def GCE_test_ifgt_true_branch(self):
        result = GCE_call_function("ifgt", [5.0, 3.0, 100.0, 200.0])
        assert result == pytest.approx(100.0)

    def GCE_test_ifgt_false_branch(self):
        result = GCE_call_function("ifgt", [1.0, 3.0, 100.0, 200.0])
        assert result == pytest.approx(200.0)

    def GCE_test_get_arity(self):
        assert GCE_get_arity("add") == 2
        assert GCE_get_arity("sin") == 1
        assert GCE_get_arity("ifgt") == 4
        assert GCE_get_arity("nonexistent") == 0

    def GCE_test_no_forbidden_functions(self):
        forbidden = {"sigmoid", "tanh", "relu", "softmax"}
